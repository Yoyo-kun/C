...
1972年，丹尼斯·里奇（Dennis Ritch）和布莱恩·柯林汉（Brian Kernighan）在开发UNIX操作系统时，同时也是 B 语言的基础上重新设计了一种新语言，这种新语言取代了 B 语言，所以称为 C 语言。
...
自顶向下的规划、结构化编程和模块化设计理念，使得代码可读性高、更可靠。
...
-   `%hd`：十进制 short int 类型。
-   `%hu`：十进制 unsigned short int 类型。
-   `%ld`：十进制 long int 类型。
-   `%lu`：十进制 unsigned long int 类型。
-   `%lld`：十进制 long long int 类型。
-   `%llu`：十进制 unsigned long long int 类型。
-   `%f`：小数（包含`float`类型和`double`类型）。
-   `%Lf`：long double 类型浮点数。
-   `%Le`：科学计数法表示的 long double 类型浮点数。
-   `%e`：使用科学计数法的浮点数，指数部分的`e`为小写。
-   `%E`：使用科学计数法的浮点数，指数部分的`E`为大写。
-   `%g`：6个有效数字的浮点数。整数部分超过6位，则会自动转为科学计数法，指数部分的`e`为小写。
-   `%G`：等同于`%g`，唯一的区别是指数部分的`E`为大写。
-   **十六进制 (Hexadecimal)  ----  x-a**
-   `%x`：十六进制整数。
...
-   `%%`：输出一个百分号。
-   **字符记录符（number）  ----  n**
-   `%n`：已输出的字符串数量。该占位符本身不输出，并将值存储在对应变量指针所指内存处。
...
-   **无符号类型（size_t)  ---- z**
-   **`%zu`**: 用于输出无符号的 `size_t` 值。
-   **`%zd`**: 用于输出有符号的 `size_t` 值（较少使用）。
-   **若系统不支持上述两种情况，则使用：**

-   **`%u`**: 如果 `size_t` 定义为 `unsigned int` 类型。
-   **`%lu`**: 如果 `size_t` 定义为 `unsigned long` 类型。
...
-   `char` : 用于表示字符，实际上是一个整数类型，可以存储一个8位的整数。

    ```
    char a = 'A'; // 或者 char a = 65;
    ```
...
-   `long` : 长整型，通常是32位或64位整数。

    ```
    long d = 2147483647L; // 使用L后缀
    ```

-   `long long` : 长长整型，通常是64位整数。

    ```
    long long e = 9223372036854775807LL; // 使用LL后缀
    ```

-   `unsigned` : 无符号整型，可以与上述类型结合使用，如 `unsigned int` ，表示只能存储非负整数。

    ```
    unsigned int f = 4294967295U; // 使用U后缀
    ```
...
-   `long double` : 长双精度浮点型，通常是扩展精度（依赖于实现，可能是80位、96位或128位）。

    ```
    long double i = 3.141592653589793238462643383279L; // 使用L后缀
    ```

#### 类型说明符

可以在上述类型前加上`unsigned`或`signed`（默认是`signed`），例如：
...
`printf()`的进制相关占位符如下：（octal、decimal、hexadecimal）
...
if (i - j < 0) // 错误
  ...
  printf("positive\n");
/* 这里与下面的整数类型的运算相区分，这里的 unsigned int 是大于 int 的类型，因此，在执行比较运算时，不会自动转换为 int 类型，而对于 unsigned char、unsigned short 这两种类型，则会出现自动转化为 int 类型从而避开了这个错误的情况。 */
```
...
所以，最好避免无符号整数与有符号整数的混合运算（比较运算、算术运算、条件运算，即三元运算、混合类型的数组索引，即如下例所示）。
...
因为这时 C 语言会自动将`signed int`转为`unsigned int`，可能不会得到预期的结果。
...
上面示例中，变量`print_ptr`是一个函数指针，它指向函数`print()`的地址。函数`print()`的地址可以用`&print`获得。注意，`(*print_ptr)`一定要写在圆括号里面，否则函数参数`(int)`的优先级高于`*`，整个式子就会变成`void* print_ptr(int)`。再给一个例子：
...
// 这里就是将上述函数换成一个函数指针，因为函数指针就是函数本身
...
上面示例可以清晰地表明，函数`compute()`的第一个参数也是一个函数，具体应用场景如下：
...
// 定义compute函数，它接受一个函数指针和两个整数作为参数，函数指针作为一种运算元，用于函数复用
...
int main()
{
    ...
    std::cout << "Result of compute with add: " << result1 << std::endl; // 输出 14 (3+3 + 4+4)

    // 使用multiply函数作为参数
    int result2 = compute(multiply, x, y);
    std::cout << "Result of compute with multiply: " << result2 << std::endl; // 输出 25 (3*3 + 4*4)
...
上面示例中，`exit()`执行时会先自动调用 `atexit()` 注册的 `print()` 函数，然后再终止程序，因此一般 `atexit` 函数用于作为清理函数的调用，下面给出实例：
...
-   **释放资源**：在程序运行过程中可能会分配资源，如内存、文件句柄、网络连接等。清理函数负责在程序结束时释放这些资源，防止资源泄漏。
-   **常见操作**：包括关闭文件、释放内存、断开网络连接、刷新缓冲区等。

**2.逆序调用的原因**：

-   **确保资源释放顺序正确**：系统按逆序调用清理函数，确保资源的释放顺序符合依赖关系。例如，先释放依赖的资源，再释放被依赖的资源，以避免依赖资源已经被释放但还在使用的情况。

**3.注册清理函数**：

-   **概念**：注册清理函数意味着将其添加到一个内部列表中，系统会在程序结束时自动调用这些函数。
-   **实现**：例如，`atexit` 函数用于注册清理函数。当程序正常结束时，`atexit` 注册的清理函数会被依次调用。

## 说明符

C 语言提供了一些说明符，让函数用法更加明确。
...
`extern` 的作用不局限于函数，下面细致地讲一讲：

1.   变量的作用

当你在一个文件中声明一个变量时，使用 `extern` 说明符表明该变量在其他源文件中定义，并非默认。例如：
...
在 `file1.c` 中，`globalVar` 被定义并分配内存。在 `file2.c` 中，通过 `extern` 说明符声明 `globalVar`，使得 `file2.c` 可以访问 `file1.c` 中定义的 `globalVar`。

2.   函数的作用

`extern` 说明符用于声明函数时，通常编译器会默认将函数声明为 `extern`，因此可以省略。例如：

```
c复制代码// file1.c
void myFunction(); // 函数声明，默认是 extern

// file2.c
extern void myFunction(); // 函数声明
```

在 `file1.c` 中定义 `myFunction`，在 `file2.c` 中使用 `extern` 声明 `myFunction`，表明它在其他文件中定义。
...
C/C++ 都可以使用 `static` 关键字来定义仅在当前源文件内部可见的变量或函数。这样，它们在其他源文件中不可见，也就避免了全局命名冲突，例如：
...
// file2.c
static int myVar = 20;  // 不会与 file1.c 中的 myVar 冲突
```
...
C 语言 和 C++ 语言 中的 `static` 语义是不一样的：

C 语言中， `static` 用于对变量进行仅一次赋值，且仅当前文件可见的定义；对函数进行仅当前文件可见的定义；

C++ 语言中，`static` 用于类的共享变量的仅一次赋值；对函数而言，是一种类整体特性体现的函数，它属于类，而不属于类的实例，可以通过类和类的实例调用，但后者不推荐；
...
上面示例中，`va_list ap`定义`ap`为可变参数对象，`va_start(ap, i)`将参数`i`后面的参数统一放入`ap`，`va_arg(ap, double)`用来从`ap`依次取出一个参数，并且指定该参数为 double 类型，`va_end(ap)`用来清理可变参数对象。给一个使用的小例子：
...
int main(void)
{
	// 可以实现由 int 类型 自动转换为 double
	double a = 1;
	printf("auto: %f\n", a);											// auto: 1.000000

	// 不能实现 int 类型 自动转换为 double
	printf("average: %f\n", average(2, 3, 1, 2, 1, 2, 3));				// average: 0.000000
	printf("average: %f\n", average(2, 3, 1.0, 2.0, 1.0, 2.0, 3.0));	// average: 3.500000

	/* 类型指定：在 va_arg(args, int) 和 va_arg(args, double) 中，
	   第二个参数（int 和 double）是类型指定，告诉编译器当前提取的参数的类型。
	   这是必需的，因为 va_list 不保存参数的类型信息。*/
	/* 类型匹配：必须确保 va_arg 使用的类型与实际参数的类型匹配。如果不匹配，结果可能是未定义的行为或错误数据。*/
	/* 可变参数的顺序：由于 va_list 是按顺序提取参数的，所以你需要知道参数的类型和顺序，以正确地使用 va_arg。 */
...
void* p = &x; 	// 整数指针隐式转为 void 指针，并赋值给 p
int* q = p; 	// void 指针隐式转为整数指针，并赋值给 q
```
...
`malloc()` 函数用于分配内存，该函数向系统要求一段内存，系统就在“堆”里面分配一段连续的内存块给它。它的原型定义在头文件`stdlib.h`。(`memory allocation` 的缩写)
...
`calloc()`函数的作用与`malloc()`相似，也是分配内存块。该函数的原型定义在头文件`stdlib.h`。(`contiguous allocation` 的缩写)
...
`realloc()`函数用于修改已经分配的内存块的大小，可以放大也可以缩小，返回一个指向新的内存块的指针。如果分配不成功，返回 NULL。该函数的原型定义在头文件`stdlib.h`。(`re-allocte` 的缩写)
...
声明指针变量时，可以使用`restrict`说明符，告诉编译器，该块内存区域只有当前指针一种访问方式，其他指针不能读写该块内存。这种指针称为“受限指针”（restrict pointer）。(`restrict` 是一种类型限定符)
...
所谓基本字符，指的是所有可打印的 ASCII 字符，但是有三个字符除外：`@`、`$`、```c
